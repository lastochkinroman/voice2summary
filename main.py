...
async def handle_audio_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not await processor.convert_to_wav(original_file, wav_file):
            await message.reply_text("❌ Ошибка при конвертации аудио.")
            return
        await message.reply_text("🔍 Распознаю речь...")
        recognizer = SpeechRecognizer()
        transcript = await recognizer.recognize(wav_file)
        if not transcript:
...
Отправьте мне голосовое сообщение или аудиофайл с записью встречи, и я:
1. 🎵 Распознаю речь через SaluteSpeech
...
Просто отправьте голосовое сообщение и получите анализ!
    """
...
2. **Отправка**: Отправьте аудио сообщение боту
...
def main():
    required_vars = ['TELEGRAM_BOT_TOKEN', 'MISTRAL_API_KEY', 'SALUTE_SPEECH_TOKEN']
